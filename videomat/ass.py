r"""
videomat.ass — port make_ass.py v2 (logika animacji bez zmian) — Motion-graphics style animated .ass generator for vertical
...
Usage (Python):
    from videomat.ass import build_ass, write_ass
    write_ass(spec_dict, 'subs.ass')
...
def build_ass(spec: dict) -> str:
    """spec (dict wg docstringu) -> treść pliku .ass."""
...
def write_ass(spec: dict, out_path: str | Path) -> Path:
    out_path = Path(out_path)
    out_path.write_text(build_ass(spec), encoding="utf-8")
    return out_path
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) != 3:
        sys.exit("Usage: python -m videomat.ass spec.json output.ass")
    write_ass(load_spec(sys.argv[1]), sys.argv[2])
